        ...
        <section class="">
            ...
            <div class="flex flex-column pb-192 border-bottom justify-between md:flex-row gap-192 mb-4">
                <p id="tabDescription" class="text-black fs-192 fw-bold">
                    <a href="{{ route('clientupdated') }}">Clients Group</a>
                    <span class="fs-118 gray-94">»</span>
                    <span class="">Edit Clients Group</span>
                </p>
            </div>


           <form action="{{ route('ClientsGroupNew.update',[ $item->id]) }}" method="POST"
                ...
                <fieldset class="floating-label-input">
                    <input type="text" name="name" value="{{ $item->name }}" id="group_name" required="">
                ...
                <div class="custom-fieldset" data-select2-id="select2-data-19861-f9jw">
                    ...
                    <select class="form-control " required name="calculation_method" id="calculation_method" style="width: 100%;"
                      >
                        ...
                        <option value="area_to_area" @if($item->calculation_method == "area_to_area") selected @endif>Area to Area</option>
                        <option value="per_area" @if($item->calculation_method == "per_area") selected @endif>Per Area</option>
                        <option value="city_to_city" @if($item->calculation_method == "city_to_city") selected @endif>City to City</option>
                        <option value="per_stop" @if($item->calculation_method == "per_stop") selected @endif>Per Stop</option>
                        <option value="per_km" @if($item->calculation_method == "per_km") selected @endif>Per km</option>
                        <option value="formula" @if($item->calculation_method == "formula") selected @endif>Formula</option>
                        <option value="flat_rate" @if($item->calculation_method == "flat_rate") selected @endif>Flat Rate</option>

                    </select>

                </div>
                <fieldset class="floating-label-input">
                    <input type="number" step="any" value="{{ $item->default_delivery_fee }}" id="default_delivery_fee" name="default_delivery_fee"
                        required="" >
                    <legend>Default delivery fee<span class="text-danger">*</span></legend>
                </fieldset>
                <fieldset class="floating-label-input">
                    <input type="number" step="any"  id="collection_amount" value="{{ $item->collection_amount }}" name="collection_amount" required="" >
                ...
                <div class="custom-fieldset" data-select2-id="select2-data-19873-uold">
                    ...
                    <select
                      ...
                      >
                        <option ></option>
                        <option value="Delivery" @if($item->service_type == "Delivery") selected @endif>Delivery</option>
                ...
                <div id="calcMethod">
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-md-6">
                            <fieldset class="floating-label-input">
                                <input name="collection_amount" value="{{ $item->collection_amount }}" type="number" step="any"
                                    >
                                <legend>Collection Amount <span class="text-danger">*</span></legend>
                            </fieldset>
                        </div>
