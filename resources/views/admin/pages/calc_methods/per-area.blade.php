<p>Per Area</p>
<div id="repeater-per-area">
    @if (isset($calculation_method) && !empty($calculation_method->data['city']))
        @for ($i = 0; $i < count($calculation_method->data['city']); $i++)
            <div class="row mb-2 align-items-center">
                <!-- From City -->
                <div class="col-md-3">
                    <div class="form-group mb-0">
                        <select class="form-control select2" style="width: 100%;" name="city[]">
                            <option value="" {{ isset($calculation_method->data['city'][$i]) && $calculation_method->data['city'][$i] == '' ? 'selected' : '' }}>Select City</option>
                            @foreach ($all_cities as $city)
                                <option value="{{ $city->id }}" {{ isset($calculation_method->data['city'][$i]) && $calculation_method->data['city'][$i] == $city->id ? 'selected' : '' }}> {{ $city->name }} </option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="form-group mb-0">
                        <select class="form-control select2" style="width: 100%;" name="area[]">
                            <option value="" {{ isset($calculation_method->data['area'][$i]) && $calculation_method->data['area'][$i] == '' ? 'selected' : '' }}>Select Area</option>
                            @foreach ($all_areas as $area)
                                <option value="{{ $area->id }}" {{ isset($calculation_method->data['area'][$i]) && $calculation_method->data['area'][$i] == $area->id ? 'selected' : '' }}> {{ $area->name }} </option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-3 ">
                    <fieldset class="floating-label-input">
                        <input type="text" class="form-control" name="fees[]" value="{{ isset($calculation_method->data['fees'][$i]) ? $calculation_method->data['fees'][$i] : '' }}">
