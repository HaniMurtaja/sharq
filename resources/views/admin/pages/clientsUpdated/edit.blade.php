...
@section('content')
    <div class="flex flex-col p-192 br-128 bg-gray-f9 ms-3 mt-2">
        <section class="">
            ...
            <form action ="{{ route('clientupdated.update', ['id' => $client->id]) }}" method="POST"
                ...
                <div class="d-flex flex-wrap gap-3 justify-content-between align-items-center mb-2">
                    <div class="position-relative w-50">
                        ...
                        {{-- Cropped image preview OR existing image --}}
                        <div id="croppedImage">
                            @if ($profile_url)
                                <img src="{{ asset($profile_url) }}" alt="Current Profile"
                    ...
                    <div class="d-flex justify-content-end align-items-center gap-4 create-user-options">
                        ...
                        <div class="form-switch d-flex justify-content-between align-items-center p-0">
                            <label class="form-check-label me-2" for="auto_dispatch">Auto Dispatch</label>
                            <input class="form-check-input position-relative m-0" type="checkbox" role="switch"
                                id="auto_dispatch" name="auto_dispatch" value="1"
                                {{ old('auto_dispatch', $client_detail?->auto_dispatch ?? false) ? 'checked' : '' }}>
                        </div>

                        {{-- Integration Switch --}}
                        <div class="form-switch d-flex justify-content-between align-items-center p-0">
                            <label class="form-check-label me-2" for="is_integration">Integration</label>
                            <input class="form-check-input position-relative m-0" type="checkbox" role="switch"
                                id="is_integration" name="is_integration" value="1"
                                {{ old('is_integration', $client_detail?->is_integration ?? false) ? 'checked' : '' }}>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-12">
                        <fieldset class="floating-label-input">
                            <input type="text" id="account_number" name="account_number"
                                value="{{ old('account_number', $client_detail?->account_number) }}" />
                ...
                <div class="row gap-md-0 gap-3">
                    ...
                    <div class="col-md-6 col-12">
                        <div class="modalSelectBox countryNewBranch">
                            <label class="customSelectLegend">Country</label>
                            <select class="countryAddress select2" name="country_id">
                                <option></option>
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}"
                                        {{ old('country_id', $client_detail?->country_id) == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row gap-md-0 gap-3">
                    <div class="col-md-6 col-12">
                        <div class="modalSelectBox cityNewBranch">
                            <label class="customSelectLegend">City</label>
                            <select class="cityAddress select2" name="city_id">
                                <option></option>
                                @foreach ($all_cities as $city)
                                    <option value="{{ $city->id }}"
                                        {{ old('city_id', $client_detail?->city_id) == $city->id ? 'selected' : '' }}>
                                        {{ $city->name }}</option>
                                @endforeach
                            </select>
                            @error('city_id')
                    ...
                    <div class="col-md-6 col-12">
                        <div class="modalSelectBox currencySelect">
                            <label class="customSelectLegend">Currency</label>
                            <select class="currency select2" name="currency">
                                <option></option>
                                @foreach (App\Enum\Currency::values() as $key => $value)
                                    <option value="{{ $key }}"
                                        {{ old('currency', $client_detail?->currency) == $key ? 'selected' : '' }}>
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                            @error('currency')
                    ...
                    <div class="col-md-6 col-12">
                        <fieldset class="floating-label-input">
                            <input type="number" id="defaultPreparationTime" name="default_prepartion_time"
                                value="{{ old('default_prepartion_time', $client_detail?->default_prepartion_time) }}" />
                    ...
                    <div class="col-md-6 col-12">
                        <fieldset class="floating-label-input">
                            <input type="number" id="minPreparationTime" name="min_prepartion_time"
                                value="{{ old('min_prepartion_time', $client_detail?->min_prepartion_time) }}" />
                    ...
                    <div class="col-md-6 col-12">
                        <fieldset class="floating-label-input">
                            <input type="number" id="partialPay" name="partial_pay"
                                value="{{ old('partial_pay', $client_detail?->partial_pay) }}" />
                    ...
                    <div class="col-md-6 col-12">
                        <fieldset class="floating-label-input">
                            <input type="text" id="price_order" name="price_order"
                                value="{{ old('price_order', $client_detail?->price_order) }}" />
                ...
                <div class="row gap-md-0 gap-3">
                    <div class="col-12">
                        <fieldset>
                            <textarea placeholder="Note" id="note" name="note" class="br-8px fs-112 border-b4 p-112 w-100">{{ old('note', $client_detail?->note) }}</textarea>
                            @error('note')
                                <div class="text-danger top-100 d-block fs-896px position-absolute">{{ $message }}</div>
                            @enderror
                        </fieldset>
                ...
                <div class="row gap-md-0 gap-3">
                    <div class="col-md-6 col-12">
                        <div class="modalSelectBox clientGroupCreateUser">
                            <label class="customSelectLegend">Client group</label>
                            <select class="clientGroup select2" name="client_group_id">
                                <option></option>
                                @foreach ($client_groups as $group)
                                    <option value="{{ $group->id }}"
                                        {{ old('client_group_id', $client_detail?->client_group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6 col-12">
                        <div class="modalSelectBox driverGroupCreateUser">
                            <label class="customSelectLegend">Driver group</label>
                            <select class="driverGroup select2" name="driver_group_id">
                                <option></option>
                                @foreach ($driver_groups as $group)
                                    <option value="{{ $group->id }}"
                                        {{ old('driver_group_id', $client_detail?->driver_group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div id="integration-div" style="display: none">
                    ...
                    <div class="modalSelectBox intergrationCompany">
                        <label class="customSelectLegend">Integration Company</label>
                        <select class="integrationBox select2" name="integration_id">
                            <option value=""></option>
                            @foreach ($integrations as $integration)
                                <option value="{{ $integration->id }}"
                                    {{ old('integration_id', $client_detail?->integration_id) == $integration->id ? 'selected' : '' }}>
