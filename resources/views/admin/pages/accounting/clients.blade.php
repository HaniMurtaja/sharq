...
<div class="flex flex-col p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-2">
            <button type="button" data-bs-target="#generateInvoiceModal" data-bs-toggle="modal" class="btn btn-primary">Generate Monthly Invoices</button>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr class="{{ !$client->is_active ? 'table-danger' : '' }}">
                        ...
                        <td>
                            <div class="text-sm">
                                <div><strong>Total Invoices:</strong> {{ optional($client->financial_summary)['total_invoices'] ?? 0 }}</div>
                                <div><strong>Total Amount:</strong> {{ number_format(optional($client->financial_summary)->total_amount ?? 0, 2) }} SAR</div>
                                <div><strong>Paid:</strong> <span class="text-green-600">{{ number_format(optional($client->financial_summary)->paid_amount ?? 0, 2) }} SAR</span></div>
                                <div><strong>Pending:</strong> <span class="text-orange-600">
                                    {{ number_format(
                                        (optional($client->financial_summary)->total_amount ?? 0) -
                                        (optional($client->financial_summary)->paid_amount ?? 0), 2
                                    ) }} SAR</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            @if(optional($client->financial_summary)->overdue_count > 0)
                                <span class="badge bg-danger">{{ optional($client->financial_summary)->overdue_count }} Overdue</span>
                            @else
                                <span class="badge bg-success">Up to Date</span>
                            @endif

                            @if(optional($client->financial_summary)->last_invoice_date)
                                <div class="text-sm text-gray-600 mt-1">
                                    Last: {{ \Carbon\Carbon::parse($client->financial_summary['last_invoice_date'])->format('M Y') }}
                                </div>
                            @endif
                        </td>
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li>
                                        <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                           data-bs-target="#invoiceHistoryModal" data-client-id="{{ $client->id }}">
                                            <i class="fas fa-file-invoice mr-2"></i> View Invoices
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                           data-bs-target="#generateInvoiceModal" data-client-id="{{ $client->id }}">
...
<div class="modal fade" id="invoiceHistoryModal" tabindex="-1" aria-labelledby="invoiceHistoryLabel" aria-hidden="true">
...
$(document).ready(function() {
    ...
    $('#generateInvoiceModal').on('show.bs.modal', function(event) {
    ...
    $('#invoiceHistoryModal').on('show.bs.modal', function (event) {
