...
<div class="row">
    <div class="col-md-3">
        ...
        <div class="form-floating mb-3">
            <input type='text' class="form-control datetimepicker1" id="from_date" value="{{@request()->fromtime}}" autocomplete="off"   name="fromtime"  />
    ...
    <div class="col-md-3">
        <span>Data To</span>
        <div class="form-floating mb-3">
            <input type='text' class="form-control datetimepicker1" id="to_date" value="{{@request()->totime}}" autocomplete="off"   name="totime"  />
            ...
            <select
                ...
                <option value="" >Dispatcher</option>
                    ...
                    @foreach ($getDispatcher as $getDispat)
                        <option value="{{ $getDispat->id }}"@if(request()->assigned_by ==$getDispat->id ) selected @endif>{{ $getDispat->full_name }}</option>
                    @endforeach
            ...
            <select
                ...
                @else
                    @foreach ($clients as $client)
                        <option value="{{ $client->id }}"@if(request()->client_id ==$client->id ) selected @endif>{{ $client->full_name }}</option>
                    @endforeach
                @endif
            </select>
        </label>
        ...
        <label class="flex flex-col gap-3">
            ...
            <select
                ...
                @foreach ($citys  as $ids=>$city)
                  <option value="{{$ids}}" @if(request()->city_id ==$ids ) selected @endif>{{$city}}</option>
                @endforeach
            </select>
        </label>
    </div>
