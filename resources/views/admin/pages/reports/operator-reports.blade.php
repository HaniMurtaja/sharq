...
@section('content')
    <div class="flex flex-col p-6">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input id="report_name" name= "report_name" class="form-control" placeholder="Report name"
                                style="width: 100%;">
                        </div>
                    </div>
        ...
        <div class="p-4 bg-white border rounded-lg border-gray1">
            ...
            <div class="w-full overflow-x-auto">
                <table id="history-list" class="w-full text-sm text-left text-gray-700 lg:table-fixed">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#type, #report_name').on('change keyup', function() {
                toggleButtonState();
            })
            ...
            $('#get-order-list').click(function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('save-operator-reports-history') }}',
                    data: formData,
            ...
            //     const deleteUrl = `{{ url('admin/export-orders') }}/${historyId}`;
            ...
            function initializeHistoriesDataTable() {
                ...
                $('#history-list').DataTable({
                    ...
                    "serverSide": true,
                    "ajax": {
                        "url": "{{ route('operators.get-operator-reports') }}",
                        "type": "GET",
                    ...
                    "columns": [{
                        ...
                        },
                        {
                            ...
                            "render": function(data, type, row) {
                                let exportRoute = '';
                                if (data.model === 'operator') {
                                    exportRoute = `{{ url('admin/export-operators') }}/${data.id}`;
                                } else if (data.model === 'vehicle') {
                                    exportRoute = `{{ url('admin/export-vehicles') }}/${data.id}`;
                                }


                                const form = `
                        <form action="${exportRoute}" method="POST" style="display:inline;" id="export-form-${data.id}">
                            @csrf
                            @method('GET')
                      <button
                type="submit"
