...
use App\Http\Resources\Api\AlkhawarizmiResource;
use App\Http\Resources\Api\DriverOrderCollection;
use App\Http\Resources\Api\Loginext\LoginextAlkhawarizmiResource;
...
class AlkhawarizmiRepository
{
    ...
    public function save_order(Request $request)
    {
        try {
            ...
            $pickup_id = ClientBranches::where('client_id', $user->id)->where('id', $branch_id)->first();
            ...
            $order = Order::create([
                ...
                'service_fees'      => \App\Models\ClientDetail::where('user_id', $user->id)->first()?->price_order ?? 14,
            ...
            return response()->json(new AlkhawarizmiResource($order), 201);
...
//        }
    ...
    public function get_order(Request $request, $id)
    {
        ...
        if ($order) {
            ...
            return response()->json(new AlkhawarizmiResource($order), 200);
        } else {
...
//        }
    ...
    public function new_order(Request $request)
    {
        ...
        return $this->send_response(true, 200, 'success', AlkhawarizmiResource::collection($order));
    }
    ...
    public function accept_order(Request $request)
    {
        ...
        switch ($request->status) {
            case 17:
                $orderResource = new AlkhawarizmiResource($order);
...
//                Log::info('not correct status');
        ...
        if ($client?->integration) {
            ...
            if ($webhook && $webhook->url) {
                ...
                if ($webhook->integration_company_id == 10) {
                    ...
                    $orderData     = new LoginextAlkhawarizmiResource($order);
                    ...
                    $data = [
                        ...
                        "pickupTimeETA" => Carbon::parse($order->driver_accepted_time, 'UTC')->addSeconds($order->pickup_duration),
                    ...
                    $orderData = new \App\Http\Resources\Api\americana\AlkhawarizmiResource($order);
    ...
    public function driver_orders(Request $request)
    {
        ...
        return $this->send_response(true, 200, 'success', AlkhawarizmiResource::collection($orders));
    }
    ...
    public function update_status(Request $request)
    {
        ...
        return $this->send_response(true, 200, 'success', new AlkhawarizmiResource($order));
    }
    ...
    private function verify_otp($code, $order)
    {
        ...
        try {
            ...
            $is_verified = ($order->otp == $code && $created->diffInMinutes($now) <= 2) ? 1 : 0;
            if ($is_verified) {
                $is_verified = true;
            } else {
                return $is_verified = false;
