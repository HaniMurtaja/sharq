    ...
    public function save_order(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'address_confirmed' => 'nullable|string',
            ...
            if ($request->has('pickup_id')) {
                $branch = ClientBranches::find($validated['pickup_id']);
                if (! $branch) {
                ...
                $validated['ingr_shop_name']    = $branch->client->name;
        ...
        if ($order) {
            ...
            if ($client?->integration) {
                ...
                $webhook = WebHook::where('integration_company_id', $client?->integration?->id)
                    ->where('type', 'order_cancelled')->first();
                if ($webhook && $webhook->url) {
                    $orderData = [
                        "order_id"     => $order->id,
                        'status'       => $order->status->value,
