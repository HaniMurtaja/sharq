...
class BigDataExport implements FromQuery, WithMapping, WithHeadings, WithChunkReading, ShouldQueue
{
    ...
    public function query()
    {
        ...
        $query = ExportedOrders::query()->orderByDesc('created_at');
    ...
    public function map($row): array
    {
        ...
        try {
            ...
            return [
                ...
                $row->value ?? 0,
                $row->service_fees ?? 0,
                ($row->value ?? 0) + ($row->service_fees ?? 0),
                optional($row->status)->getLabel() ?? '',
                optional($row->payment_type)->getLabel() ?? '',
                $row->driver_id,
                optional($row->DriverDataSearch)->full_name ?? '',
                @$row->ClientDetail->account_number,
                @$row->ingr_shop_id,
                @$row->shop->full_name,
                @$row->ingr_shop_id,
                @$row->branch->name,
                @$row->cityData->name,
                @$row->cancel_reason->name,
                @$row->created_at,
